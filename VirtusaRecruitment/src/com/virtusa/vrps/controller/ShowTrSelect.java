...
import com.virtusa.vrps.dao.JobSeekerDAOImpl3;
import com.virtusa.vrps.model.JobApplicationInfo;

public class ShowTrSelect extends HttpServlet {
	static Logger logger = Logger.getLogger(ShowTrSelect.class);
...
@Override
	...
	response.setDateHeader("Expires", 0);
		...
		try {
			...
			JobSeekerDAO jid = new JobSeekerDAOImpl3(con);
			List<JobApplicationInfo> jListInfo = jid.getSelectInfo();
			
				request.setAttribute("jListInfo", jListInfo);

				request.getRequestDispatcher("trselect.jsp").forward(request, response);
